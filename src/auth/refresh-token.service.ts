...
export class RefreshTokenService {
    ...
    async rotate(rawRefreshToken: string) {
        ...
        if (!tokenRecord) {
            ...
            throw new UnauthorizedException('Security Alert: Invalid token. All sessions revoked.');
        }
        ...
        const newRefresh = this.tokenService.signRefreshToken(payload.sub, payload.email);
        ...
        await this.create(payload.sub, newRefresh.token, newRefresh.jti, newExpiresAt);

        // 8. Sign new access token
        const newAccessToken = this.tokenService.signAccessToken(payload.sub, payload.email);
